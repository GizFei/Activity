...
class FragmentTransitionActivity : AppCompatActivity() {
    ...
    private fun initFragment() {
        val fragmentA = TransitionFragmentAAA().apply { arguments = Bundle().apply { putInt("index", 1) } }
    ...
    private fun initViews() {
        btn_add_fragment.setOnClickListener {
            if (!isFragmentBAdded) {
                // 添加FragmentB
                val fragmentB = TransitionFragmentBBB().apply { arguments = Bundle().apply { putInt("index", 2) } }
